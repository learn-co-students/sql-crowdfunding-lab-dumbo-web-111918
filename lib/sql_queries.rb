...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS overrun FROM projects INNER JOIN pledges ON projects.id=pledges.project_id GROUP BY Projects.title HAVING overrun >= 0"
end
